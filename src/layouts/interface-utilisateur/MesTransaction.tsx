...
const TransactionUser = () => {
          ...
          try {
            ...
            if(response.data.id){
             fetchTransactions(response.data.id)
              ...
              console.log('Transaction doesn\'t fetch')
            }
          } catch (error) {
  ...
  const closeConfirmModal = () => {
    ...
    const fetchTransactions = async (id : string) => {
                ...
                try {
                const response = await axios.get< Transaction[]>(`http://localhost:8000/api/transactions-user/${id}`);
  ...
  return (
    <Paper sx={{ px: 0 ,margin:1 ,marginTop:12}}>

        <Box mb={3} display="flex" justifyContent="space-between" alignItems="center">
            <Typography variant="h4" gutterBottom> Toutes vos transactions</Typography>
            <Stack
            px={3.5}
            spacing={1.5}
